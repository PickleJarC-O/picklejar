...
FLATPAK = io.github.PickleJarC-O.Picklejar
...
vinegar:
	$(GO) build $(VINEGAR_GOFLAGS) $(GOFLAGS) -ldflags="$(VINEGAR_LDFLAGS)" ./cmd/picklejar

robloxmutexer.exe:
	GOOS=windows $(GO) build $(GOFLAGS) -ldflags="$(GO_LDFLAGS)" ./cmd/robloxmutexer

install-picklejar: picklejar
	install -Dm755 picklejar $(DESTDIR)$(PREFIX)/bin/picklejar
...
icons: icons/roblox-player.svg icons/roblox-studio.svg
	...
	convert -density 384 -background none icons/picklejar.svg -resize 64x64 splash/picklejar.png
